# Proposal: CRCR Support for Nightly & Periodic CI

**Status:** RFC — for CRCR Working Group discussion
**Date:** June 2026
...
The callback Lambda already handles **EventBridge-triggered events** for zombie cleanup. When `event.source == "crcr.sweeper"`, it runs the cleanup handler instead of processing a callback. This is the exact pattern Option 1 would follow — an EventBridge cron rule invoking a Lambda handler.
...
### Option 1: EventBridge Cron → Webhook Lambda
...
**Downstream workflow change** — add `nightly` to dispatch types:
...
The rest of the workflow stays the same. The `delivery_id`, SHA, and `event_type` are in `client_payload`, and the callback action works unchanged.
...
| 4 | **Proven pattern** | The zombie cleanup handler already uses EventBridge → Lambda (`source: crcr.sweeper`). This is the same architecture. |
| 5 | **Central control** | The relay controls who gets nightly dispatches (allowlist-gated) and when they fire. Aligns with CRCR's trust model. |
| 6 | **State machine integrity** | Every nightly run has a real `DISPATCHED` record. No special-casing or bypasses. |
| 7 | **Schedule reliability** | EventBridge has an [SLA of 99.99%](https://aws.amazon.com/eventbridge/sla/). Far more reliable than GitHub's best-effort cron. |
| 8 | **Consistent SHA** | All downstream repos build against the same HEAD SHA fetched once at dispatch time. Cross-backend comparison is meaningful. |
...
| 5 | **SHA staleness** | The HEAD SHA is fetched once. If `main` advances between dispatch and the downstream build starting (minutes to hours later), the tested SHA is already stale. |
| 6 | **Allowlist complexity** | Needs a way to opt repos in/out of nightly dispatches (not all L1+ repos want them). Adds a new dimension to the allowlist config. |
...
| Webhook Lambda handler | New path for `source: crcr.scheduler`: fetch HEAD SHA, build synthetic payload, call `_dispatch_to_allowlist()`. ~100-150 LOC, reusing existing dispatch logic | ~1 day |
...
### Option 2: Upstream Cron Workflow in pytorch/pytorch → Webhook Lambda
...
**Downstream workflow change** — same as Option 1:
...
| 3 | **Manual trigger** | `workflow_dispatch` means anyone with repo write access can manually re-trigger a nightly dispatch from the GitHub UI — useful for debugging, hotfix verification, etc. |
| 4 | **Familiar** | It's a GitHub Actions workflow. No AWS console, no Terraform, no new infrastructure concepts for contributors. |
| 5 | **Schedule flexibility** | Multiple cron entries or separate workflows can define different schedules (nightly, weekly, periodic) without touching the relay. |
| 6 | **PR-able changes** | Changing the schedule is a PR to `pytorch/pytorch`, reviewed by maintainers with standard CI. Not a Terraform deployment. |
...
| # | Disadvantage | Detail |
|---|-------------|--------|
| 1 | **Authentication complexity** | The webhook Lambda currently validates requests using **GitHub webhook signatures** (`X-Hub-Signature-256`). A workflow-triggered POST doesn't have this signature. Requires adding a **new auth path**: either OIDC verification (like the callback Lambda uses) or a shared secret. This touches the security boundary of the webhook Lambda. |
| 2 | **Not a real GitHub webhook** | The synthetic payload mimics a webhook but isn't one. The Lambda's `_verify_signature` would reject it. The `x-github-event` header is self-set. The payload structure differs from a real `push` event. Needs a new code path in the Lambda to parse and validate this non-standard input. |
| 3 | **Requires changes in pytorch/pytorch** | Adding a workflow to the upstream repo requires buy-in from PyTorch maintainers. This is an org-level change — the CRCR team doesn't own `pytorch/pytorch`. |
| 4 | **GitHub cron unreliability** | GitHub's `schedule:` trigger is [best-effort](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#schedule). During high-load periods, cron runs can be delayed by minutes to hours or skipped entirely. Less reliable than EventBridge's 99.99% SLA. |
| 5 | **Secret management** | If using a shared secret (not OIDC), the secret must be stored in `pytorch/pytorch` repo secrets and rotated in sync with the Lambda config. If using OIDC, the webhook Lambda needs JWKS verification (currently only the callback Lambda has this). |
| 6 | **Dependency on pytorch/pytorch** | Downstream nightly scheduling now depends on an upstream workflow running. If the pytorch/pytorch Actions runner is down, quotas are exhausted, or the workflow is disabled, all downstream nightlies stop. |
| 7 | **Event handler branching** | The webhook Lambda's `event_handler.py` currently only special-cases `pull_request` (checking `action ∈ {opened, reopened, synchronize, closed}`). A `nightly` event type has neither a `pull_request` nor a standard `push` payload shape, requiring new parsing logic and a synthetic payload contract. |
| 8 | **`_SUPPORTED_EVENTS` gating** | The Lambda's `_SUPPORTED_EVENTS = {"pull_request", "push"}` rejects unknown event types before signature verification. Adding `nightly` means changing this gate **and** adding the corresponding auth path — two coupled changes in the security-critical path. |
...
| Webhook Lambda auth | New auth path: OIDC verification (port `jwt_helper` from callback Lambda) or shared-secret validation. Must coexist with existing `_verify_signature`. | ~1.5 days |
| Webhook Lambda handler | Expand `_SUPPORTED_EVENTS`, add `nightly` event parsing path (different payload shape from `pull_request`/`push`), generate `delivery_id` | ~1 day |
| Downstream workflow | Same as Option 1 | ~0.5 day |
| HUD view | Same as Option 1 | ~1 day |
...
| **Total** | | **~5.5-9.5 days** (including approval lead time) |

---

## Side-by-Side Comparison

| Criteria | Option 1: EventBridge → Lambda | Option 2: pytorch/pytorch Cron → Lambda |
|----------|:---:|:---:|
...
### Option 1 is lower effort and lower risk

**Auth is the key differentiator.** Option 1 requires zero auth changes — EventBridge invokes the Lambda directly as an internal AWS call, inherently trusted. Option 2 requires adding a second authentication mechanism to the webhook Lambda (OIDC or shared secret), which touches the security boundary and requires careful review.

**Proven architecture.** Option 1 follows the exact same EventBridge → Lambda pattern already used for zombie cleanup (`source: crcr.sweeper`). The infrastructure pattern, Terraform resources, and Lambda dispatch logic are all precedented.

**No cross-repo coordination.** Option 1 keeps everything in `pytorch/test-infra`. Option 2 requires a PR to `pytorch/pytorch`, which involves a different set of maintainers and a different review cycle.

### Option 2 has real advantages for visibility and self-service

**`workflow_dispatch` is genuinely useful.** Being able to click "Run workflow" in the GitHub UI to trigger an ad-hoc nightly dispatch — without AWS console access — is valuable for debugging and for downstream teams who want to verify a fix before the next scheduled run.

**Operational transparency.** The nightly schedule living in a workflow file (not Terraform) means it's visible, grep-able, and PR-able by anyone with repo access. This lowers the barrier for schedule changes.

**These advantages could justify the additional effort** if the WG decides that visibility and self-service are higher priorities than implementation simplicity.
...
The HUD currently groups results by `pr_number`. For nightly runs there is no PR.

- Use `pr_number = 0` as a sentinel for non-PR runs
- The `event_type` field already flows through the pipeline — it just needs to carry `nightly` or `periodic` instead of `pull_request`
- Add a filter/view on `hud.pytorch.org/crcr` for non-PR results, or a dedicated `/crcr/nightly` page showing results by date + backend

### Allowlist Scoping

Not all L1+ repos should receive nightly dispatches. Consider:
...
For nightly runs, which SHA to test against?
...
PR failures are visible on the PR. Nightly failures have no PR to annotate.
