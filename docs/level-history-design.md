...
History is an additional, collapsible panel in the existing repository dashboard, not a replacement dashboard or a PR/nightly tab. It is rendered directly after L3 Readiness and before the current KPI cards. Both panels are collapsed initially and use the same title, compact Details chip, and right-aligned summary-chip pattern. The panel supplies one filterable chronological decision history, a decision-specific supporting signal, and evidence; the existing PR matrix remains unchanged below the current dashboard cards.
